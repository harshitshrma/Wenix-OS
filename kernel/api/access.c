...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        list_iterate_begin(&map->vmm_list, find, vmarea_t, vma_plink)
            {   
                if (find == NULL)
                {
                    dbg(DBG_TEMP, "addr_perm exit\n");
                    dbg(DBG_PRINT, "(GRADING3B X)\n");
                    return 0;
                }
                if(find->vma_end > ADDR_TO_PN(vaddr) && find->vma_start <= ADDR_TO_PN(vaddr))
                {
                   if((find->vma_prot & perm) != perm)
                   {
                        dbg(DBG_TEMP, "addr_perm exit\n");
                        dbg(DBG_PRINT, "(GRADING3B X)\n");
                        return 0;
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        list_iterate_begin(&map->vmm_list, find, vmarea_t, vma_plink)
            {   
                if (find == NULL)
                {
                    dbg(DBG_TEMP, "range_perm exit\n");
                    dbg(DBG_PRINT, "(GRADING3B X)\n");
                    return 0;
                }
                if(find->vma_end > ADDR_TO_PN((uint32_t)avaddr + len) && find->vma_start <= ADDR_TO_PN(avaddr))
                {
                    if((find->vma_prot & perm) != perm)
                    {
                        dbg(DBG_TEMP, "range_perm exit\n");
