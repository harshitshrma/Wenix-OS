...
sys_read(read_args_t *arg)
{
        ...
        dbg(DBG_TEMP, "sys_read entry\n");
        read_args_t new;
        int cfw = copy_from_user(&new, arg, sizeof(read_args_t)); 
        if (cfw < 0)
        {
            curthr->kt_errno = -cfw;
            dbg(DBG_TEMP, "sys_read exit\n");
        ...
        int error = do_read(new.fd,new.buf,new.nbytes);
        if(error < 0)
        {
        ...
        int ctw = copy_to_user(arg->buf, new.buf, new.nbytes);
        if (ctw < 0)
        {
            curthr->kt_errno = -ctw;
            page_free(new.buf);
...
sys_write(write_args_t *arg)
{
        ...
        dbg(DBG_TEMP, "sys_write entry\n");
        write_args_t new;
        int cfw = copy_from_user(&new, arg, sizeof(write_args_t)); 
        if (cfw < 0)
        {
            curthr->kt_errno = -cfw;
            dbg(DBG_TEMP, "sys_write exit\n");
            dbg(DBG_PRINT, "(GRADING3B X)\n");
            return -1;
        }
        new.buf = page_alloc();
        ...
        int ctw = copy_from_user(new.buf,arg->buf, arg->nbytes);
        if (ctw < 0)
        {
            curthr->kt_errno = -ctw;
            page_free(new.buf);
        ...
        int error = do_write(new.fd,new.buf,new.nbytes);
        if(error < 0)
        {
            curthr->kt_errno = -error;
            page_free(new.buf);
...
sys_getdents(getdents_args_t *arg)
{
        ...
        getdents_args_t new;
        int cfw = copy_from_user(&new, arg, sizeof(getdents_args_t));
        if (cfw < 0)
        {
            curthr->kt_errno = -cfw;
            dbg(DBG_PRINT, "(GRADING3B X)\n");
        ...
        dirent_t d; 
        int temp = 0;
        for(uint32_t i = 0; i < new.count/sizeof(dirent_t);i++)
        {
            error = do_getdent(new.fd,&d);
            if(error < 0)
            {
                curthr->kt_errno = -error;
                dbg(DBG_PRINT, "(GRADING3B X)\n");
            ...
            {
                copy_to_user(new.dirp + i, &d, error);
                temp = temp + error;
