...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        { 
            ...
            dbg(DBG_PRINT, "(GRADING2A 1.a)\n");
            bytedev_t *ret = bytedev_lookup(file->vn_devid);
            if (ret != NULL)
            {
                dbg(DBG_PRINT, "(GRADING2B)\n");
                int read_fs = ret->cd_ops->read(ret, offset, buf, count);
                dbg(DBG_TEMP, "sfr exit\n");
                return read_fs;
            }
        }
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
        ...
        if (S_ISCHR(file->vn_mode)) 
        {
            ...
            {
                dbg(DBG_PRINT, "(GRADING2B)\n");
                int write_fs = ret->cd_ops->write(ret, offset, buf, count);
                dbg(DBG_TEMP, "sfw exit\n");
                return write_fs;
            }
        }
        dbg(DBG_PRINT, "(GRADING2B)\n");
...
special_file_mmap(vnode_t *file, vmarea_t *vma, mmobj_t **ret)
{
        ...
        dbg(DBG_TEMP, "special_file_mmap entry\n");
        bytedev_t *byte = bytedev_lookup(file->vn_devid);
        dbg(DBG_TEMP, "special_file_mmap exit\n");
        dbg(DBG_PRINT, "(GRADING3B X)\n");
        return byte->cd_ops->mmap(file, vma, ret);
}
...
special_file_fillpage(vnode_t *file, off_t offset, void *pagebuf)
{
        ...
        dbg(DBG_TEMP, "special_file_fillpage entry\n");
        bytedev_t *byte = bytedev_lookup(file->vn_devid);
        dbg(DBG_TEMP, "special_file_fillpage exit\n");
        dbg(DBG_PRINT, "(GRADING3B X)\n");
        return byte->cd_ops->fillpage(file, offset, pagebuf);
}
...
special_file_dirtypage(vnode_t *file, off_t offset)
{
        ...
        dbg(DBG_TEMP, "special_file_dirtypage entry\n");
        bytedev_t *byte = bytedev_lookup(file->vn_devid);
        dbg(DBG_TEMP, "special_file_dirtypage exit\n");
        dbg(DBG_PRINT, "(GRADING3B X)\n");
        return byte->cd_ops->dirtypage(file, offset);
}
...
special_file_cleanpage(vnode_t *file, off_t offset, void *pagebuf)
{
        ...
        dbg(DBG_TEMP, "special_file_cleanpage entry\n");
        bytedev_t *byte = bytedev_lookup(file->vn_devid);
        dbg(DBG_TEMP, "special_file_cleanpage exit\n");
        dbg(DBG_PRINT, "(GRADING3B X)\n");
        return byte->cd_ops->cleanpage(file, offset, pagebuf);
}
