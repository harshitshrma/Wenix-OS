...
class InfoCommand(wenix.Command):
    ...
    def __init__(self):
        wenix.Command.__init__(self, "info",
                                gdb.COMMAND_DATA,
                                gdb.COMPLETE_SYMBOL)
    ...
    def invoke(self, arg, tty):
        ...
        gdb.write(wenix.info.string(args[0], args[1] if (len(args) > 1) else None))

InfoCommand()

class DbgCommand(wenix.Command):
    """usage: dbg [<modes>]
    ...
    def __init__(self):
        wenix.Command.__init__(self, "dbg",
                                gdb.COMMAND_DATA)
    ...
    def invoke(self, arg, tty):
        if (len(arg.strip()) == 0):
            info = wenix.info.string("dbg_modes_info")
            if (len(info) == 0):
                gdb.write("No active modes.\n")
            else:
                gdb.write("{0}\n".format(wenix.info.string("dbg_modes_info")))
        else:
            modes = self._modes()
            for mode in arg.split():
                ...
                if (not name in modes):
                    ...
                    wenix.eval_func("dbg_add_mode", "\"{0}\"".format(mode))
