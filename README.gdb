...
You can debug Wenix with gdb by passing the '-d gdb' argument to
the Wenix script when running a supported platform. Note that gdb
is not aware of Wenix threads and processes, so you cannot use
the "thread" command (or similar commands) to change the thread
being debugged, you must allow Wenix to run until the thread you
want to debug becomes the current thread.

Extensions
========================

Wenix provides several custom gdb commands for tasks such as
...
kernel proc:
Prints information about Wenix processes.
...
Extensions are written as python modules in the Wenix module with
...
target searches all Wenix modules for files with a .py extension
...
extensions. However, Wenix has a set of common extension modules
...
API to very Wenix specific API's. When possible you should always
use the Wenix specific wrappers around gdb's APIs. The following
are descriptions of common uses of Wenix's modules.

== Custom Commands ==

The most likely thing you will want to do is add new commands
(e.g. 'break', 'backtrace', 'info breakpoints', etc.) to gdb. The
gdb API provides the gdb.Command class for this, however Wenix
has a wenix.Command class which you should use instead. The
wenix class automatically prefixes your command with the word
...
tab complete to see a list of available Wenix specific commands
or run the 'kernel' command by itself to get the same
list. Otherwise wenix.Command is identical to gdb.Command.
...
you should use the wenix.stack.Frame class. This class unwinds
...
of Wenix. In order to standardize the way this is done we provide
...
however, just as with commands, Wenix has its own
wenix.Parameter class which prefixes all parameters with 'kernel'
(e.g. 'set kernel memcheck'). Also as with commands, users can run
'show kernel' to see a list of Wenix specific parameters. A
common type of parameter is one that is set once, before Wenix
boot, and should never be changed again while Wenix runs. Wenix
provides a special class for this called wenix.Flag. These
