...
Is the following statement correct about your submission (please replace
    ...
    We run /sbin/init from the kernel init process in our submission": no, Use Kshell to run the below tests.
...
1)  Names and USC e-mail addresses of team members: 
	Aditya Prabhakar Balagere 	- abalager@usc.edu
    	Karun Satish          		- ksatish@usc.edu
    	Ankita Dindayal Agarwal   	- ankitadi@usc.edu
    	Harshit Sharma        		- harshit@usc.edu
...
Please skip the following tests: Skip tests	1. cat /README i
						2. /bin/ls
						3. /usr/bin/vfstest
						4. /usr/bin/memtest
						5. /usr/bin/eatmem
...
+---------------------------------------------------------------------------------------+
Run the tests in (B) in Kshell with the following commands
(1) /usr/bin/hello 			- hello
(2) /usr/bin/args ab cde fghi j		- args
(3) /bin/uname -a			- uname
(4) /bin/stat /README			- readme
(5) /bin/stat /usr			- user
(6) /bin/ls /usr/bin			- ls
(7) /usr/bin/fork-and-wait		- fork-and-wait

Run the tests in (C) in Kshell with the following commands
Type sbin to get user shell in kshell and type the following commands
(1) help
(2) echo hello
(5) segfault
	
Run the tests in (D) in Kshell with the following commands
Type forkbomb to start the forkbomb test and exit to halt it (we won't be able the see shell prompt properly but if we type exit it will halt) , but the weenix is not halting cleanly, Refcount of 1 vnode is leaking.
+-----------------------------------------------+
