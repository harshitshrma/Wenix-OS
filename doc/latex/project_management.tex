...
\item \texttt{gdb}, the GNU debugger, and \texttt{xterm} % TODO "Wenix includes special extensions to gdb writen using gdb's python extension api. In order to use these extensions you must be using at least gdb version 7.3"
\item \texttt{cscope} (for easier browsing of the Wenix source)
\end{itemize}

% TODO restore this section once bochs/vbox support is fixed in ./wenix
% For hardware virtualization, Wenix can use QEMU, Bochs, or VirtualBox.  We recommend QEMU, for its relative ease of setup and (we've found) superior simulation speed and quality.  You may need a special build of Bochs in order to use it with Wenix; feel free to contact \texttt{wenix-devel@lists.cs.brown.edu} if Wenix doesn't seem compatible with your installation of Bochs.
...
At some point, it may be helpful to use a debugger to aid in the development of the operating system. The following command will begin Wenix in a virtual machine and attach \texttt{gdb} to it (if the virtual machine in use supports this option).
\begin{verbatim}
$ ./wenix -d gdb\end{verbatim}

%Finally, the script supports several different virtual machines and simulators.
%\begin{verbatim}
%$ ./wenix -m qemu   % the default
%$ ./wenix -m bochs
%$ ./wenix -m vbox\end{verbatim}
